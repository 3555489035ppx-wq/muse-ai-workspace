...
export function SettingsPage() {
  ...
  const providerManagedBySite = Boolean(providerFromApi.managedBySite);
            ...
            {tab === "provider" ? (
              <>
                ...
                {activeDraft ? <Surface title={`${providerCategory === "text" ? "Text AI" : "Image AI"} Provider 配置`} className="provider-config-surface" action={<StatusPill status={providerStatusTone(activeProvider)}>{providerStatusLabel(activeProvider)}</StatusPill>}>
                  ...
                  {providerManagedBySite ? <div className="provider-test-result"><CheckCircle2 size={16}/><span>此服务由 Muse 站点托管，已安全连接。你无需提供、查看或修改 API Key。</span></div> : null}
                  <div className="provider-form-grid">
                    <Field label="AI Provider" hint="可切换，Custom 用于 OpenAI-compatible API"><select disabled={providerManagedBySite} value={activeDraft.provider} onChange={(event) => updateProviderDraft("provider", event.target.value)}>{providerOptions.map((option) => <option value={option.value} key={option.value}>{option.label} · {option.text}</option>)}</select></Field>
                    <Field label="模型 ID" hint="可直接填写服务商提供的模型名称"><input disabled={providerManagedBySite} value={activeDraft.modelId} onChange={(event) => updateProviderDraft("modelId", event.target.value)} placeholder={providerCategory === "text" ? "deepseek-v4-pro" : "gpt-image-2"}/></Field>
                    <Field label="API Base URL" hint="仅允许 HTTPS，或本机 localhost / 127.0.0.1"><input disabled={providerManagedBySite} value={activeDraft.baseUrl} onChange={(event) => updateProviderDraft("baseUrl", event.target.value)} placeholder="https://api.example.com/v1" spellCheck="false"/></Field>
                    <div className="provider-secret-field"><span className="field__label">API Key</span><span className="field__hint">{providerManagedBySite ? "由站点安全托管，完整密钥不会发送到浏览器。" : "完整密钥只发送给 Muse 安全 API，不写入浏览器持久化数据。"}</span><div className="provider-secret-control"><code>{providerManagedBySite ? "由站点托管" : activeProvider?.keyHint ?? "尚未添加密钥"}</code><Button disabled={providerManagedBySite} variant="quiet" icon={KeyRound} onClick={openProviderKeyModal}>{activeProvider?.keyHint ? "更换密钥" : "添加密钥"}</Button></div></div>
                  </div>
                  {providerCategory === "text" ? <Field label="推理模式" hint="DeepSeek 支持 high / max；其他兼容服务会按接口能力处理"><select disabled={providerManagedBySite} value={activeDraft.reasoningMode} onChange={(event) => updateProviderDraft("reasoningMode", event.target.value)}><option value="standard">标准</option><option value="high">高</option><option value="max">最大</option></select></Field> : <div className="provider-form-grid"><Field label="默认质量"><select disabled={providerManagedBySite} value={activeDraft.quality} onChange={(event) => updateProviderDraft("quality", event.target.value)}><option value="standard">标准</option><option value="high">高</option></select></Field><Field label="默认比例"><select disabled={providerManagedBySite} value={activeDraft.aspectRatio} onChange={(event) => updateProviderDraft("aspectRatio", event.target.value)}><option value="square">方形</option><option value="landscape">横向</option><option value="portrait">竖向</option></select></Field></div>}
                  <button type="button" disabled={providerManagedBySite} className="provider-advanced-toggle" onClick={() => setProviderAdvanced((value) => !value)}><span>高级设置</span><small>显示名称、自定义模型 ID 与运行开关</small><ChevronDown size={16} className={providerAdvanced ? "is-open" : ""}/></button>
                  {providerAdvanced ? <div className="provider-form-grid provider-form-grid--advanced"><Field label="显示名称"><input disabled={providerManagedBySite} value={activeDraft.displayName} onChange={(event) => updateProviderDraft("displayName", event.target.value)} placeholder="例如：我的 Text AI"/></Field><Field label="Custom Model ID" hint="填写后优先使用此 ID"><input disabled={providerManagedBySite} value={activeDraft.customModelId} onChange={(event) => updateProviderDraft("customModelId", event.target.value)} placeholder="可选"/></Field><label className="provider-enabled-toggle"><input disabled={providerManagedBySite} type="checkbox" checked={activeDraft.enabled} onChange={(event) => updateProviderDraft("enabled", event.target.checked)}/><span><strong>允许 Muse 使用这个 Provider</strong><small>关闭后不会发起真实请求，也不会生成实时结果。</small></span></label></div> : null}
                  ...
                  <div className="provider-config-actions"><Button variant="quiet" icon={RefreshCw} loading={activeTest.status === "testing"} disabled={providerManagedBySite || activeTest.status === "testing"} onClick={() => void testCurrentProvider()}>{providerManagedBySite ? "由站点维护连接" : activeTest.status === "testing" ? "正在真实调用…" : "测试真实连接"}</Button><Button icon={Check} loading={activeTest.status === "saving"} disabled={providerManagedBySite} onClick={() => void saveCurrentProvider()}>保存配置</Button>{activeProvider?.keyHint && !providerManagedBySite ? <Button variant="danger" onClick={() => setConfirmProviderDelete(providerCategory)}>删除配置</Button> : null}</div>
