...
function ResearchAssistantPanel({ workspace }) {
  ...
  return <section className="research-v2__assistant" aria-label="AI 研究助手"><header><div><p className="industrial-kicker">AI RESEARCH ASSISTANT</p><h2>先把“我该查什么”变成可执行线索</h2><p>真实 Text AI 会根据已确认简报拆解研究路径。它不联网、不写入来源，也不会自动采纳任何事实。</p></div><ResearchBadge tone={tone}>{statusCopy}</ResearchBadge></header>
    ...
    {!assistant.questionPlans?.length && assistant.status !== "error" ? <div className="research-v2__assistant-empty"><div><strong>点击上方“生成研究计划”</strong><p>Muse 会为每个研究问题给出检索词、优先来源类型和证据标准，帮助你从模糊想法开始查到真正相关的材料。</p></div><ol><li><span>01</span><strong>AI 拆解问题</strong></li><li><span>02</span><strong>你打开真实来源</strong></li><li><span>03</span><strong>确认后进入洞察</strong></li></ol></div> : null}
    {assistant.questionPlans?.length ? <div className="research-v2__assistant-plans">{assistant.questionPlans.map((plan, index) => <article key={plan.id}><div className="research-v2__assistant-plan-heading"><span>RQ{String(index + 1).padStart(2, "0")}</span><h3>{plan.question}</h3></div><div className="research-v2__assistant-plan-copy"><div><span>为什么查</span><p>{plan.whyThisMatters}</p></div><div><span>需要什么证据</span><p>{plan.evidenceNeed}</p></div></div><div className="research-v2__assistant-queries"><span>建议检索词</span><div>{plan.querySuggestions.map((query) => <button type="button" key={query} onClick={() => openSearch(query)}>{query}<ExternalLink size={12}/></button>)}</div></div><div className="research-v2__assistant-sources"><span>优先来源类型</span><p>{plan.preferredSources.join(" · ")}</p></div></article>)}</div> : null}
    ...
    <footer className="research-v2__assistant-note"><Sparkles size={15}/><p>{assistant.note}</p><strong>打开检索词 → 添加来源 → 粘贴原文 → 你确认采纳 → 生成 Design Insight</strong></footer>
  </section>;
}
...
export function IndustrialResearchPage() {
  ...
  return <Page project={project} eyebrow="02 · 研究证据" title="找到真正会影响设计判断的信息" description="Muse 先帮你把模糊想法拆成研究线索，再把你确认过的真实来源推进到设计洞察。" showWorkflowConversion={false} actions={<div className="research-v2__header-actions"><button className="industrial-button" onClick={() => setDrawer(true)}><Link2 size={15}/>添加来源</button><PrimaryButton disabled={!insightGate.ready} onClick={() => void enterInsight(false)}>用 {accepted.length} 条证据生成设计洞察</PrimaryButton></div>}>
    ...
    <ResearchAssistantPanel workspace={workspace}/>
    {brief?.status !== "confirmed" && !industrial?.briefConfirmed ? <div className="industrial-gate"><LockKeyhole size={18}/>当前简报还未确认；先确认简报，研究问题才会冻结。</div> : null}
