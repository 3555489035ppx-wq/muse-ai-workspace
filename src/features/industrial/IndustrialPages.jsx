...
function Page({ project, eyebrow, title, description, children, actions, showWorkflowConversion = true, showAiToolbar = true }) {
  ...
  const generateResearchPlan = useMuseStore((state) => state.generateIndustrialResearchPlan);
  ...
  const aiAction = stage === "brief" ? generateBrief : stage === "research" ? generateResearchPlan : stage === "insight" ? generateInsights : stage === "direction" ? generateDirections : stage === "concept" ? generateConcepts : stage === "cmf" ? generateCmf : null;
  const aiLabel = stage === "brief" ? "整理 Brief" : stage === "research" ? "生成研究计划" : stage === "insight" ? "重新生成洞察" : stage === "direction" ? "重新生成方向" : stage === "concept" ? "重新生成概念" : stage === "cmf" ? "重新生成 CMF" : "";
...
function ResearchPlan({ workspace, onClose }) {
  return <div className="research-v2__plan"><header><div><p className="industrial-kicker">RESEARCH PLAN</p><h3>先生成研究线索，再补充真实来源</h3></div><button className="research-v2__icon-button" onClick={onClose} aria-label="关闭研究计划"><X size={16}/></button></header><ol>{workspace.plan.map((item, index) => <li key={item.id}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{item.label.replace("围绕 ", "")}</strong><small>AI 会生成检索词与证据标准，但不会把推断当成来源。</small></div></li>)}</ol><footer><button className="industrial-button" onClick={onClose}>先调整问题</button><button className="industrial-button industrial-button--primary" onClick={onClose}>查看 AI 研究助手</button></footer></div>;
}

function ResearchAssistantPanel({ workspace, projectId }) {
  const searchSources = useMuseStore((state) => state.searchResearchSources);
  const importSearchResult = useMuseStore((state) => state.importResearchSearchResult);
  const [activeSearch, setActiveSearch] = useState("");
  const [searchCapability, setSearchCapability] = useState(null);
  useEffect(() => {
    const controller = new AbortController();
    void aiClient.capabilities(controller.signal).then((capabilities) => setSearchCapability(capabilities?.providers?.search ?? null)).catch((error) => {
      if (error?.name !== "AbortError") setSearchCapability(null);
    });
    return () => controller.abort();
  }, []);
  const assistant = workspace.researchAssistant ?? { status: "idle", source: "none", questionPlans: [], gaps: [], nextActions: [], note: "AI 只生成研究线索与执行计划，不代表已经找到或验证真实来源。" };
  const searchState = workspace.researchSearch ?? { status: "idle", query: "", questionId: null, provider: null, results: [], errorMessage: null, note: "搜索结果只会导入候选来源，不会自动成为已验证证据。请打开原文核对后再采纳。" };
  const importedUrls = new Set((workspace.sources ?? []).map((source) => source.sourceUrl).filter(Boolean));
  const statusCopy = assistant.status === "success" ? "已生成研究线索" : assistant.status === "partial" ? "部分问题已生成" : assistant.status === "error" ? "本轮生成失败" : "等待生成";
  const tone = assistant.status === "success" ? "success" : assistant.status === "partial" ? "candidate" : assistant.status === "error" ? "danger" : "neutral";
  const openManualSearch = (query) => {
    const url = "https://www.baidu.com/s?wd=" + encodeURIComponent(query);
    window.open(url, "_blank", "noopener,noreferrer");
  };
  const runSearch = async (query, questionId) => {
    setActiveSearch(query);
    try { await searchSources(projectId, query, questionId); } finally { setActiveSearch(""); }
  };
  const searchUnavailable = searchCapability !== null && searchCapability?.ready !== true;
  const searchStatus = searchState.status === "searching" ? "正在搜索真实来源…" : searchState.status === "success" ? `已找到 ${searchState.results.length} 条候选` : searchState.status === "empty" ? "搜索完成，暂无可安全导入的来源" : searchState.status === "error" ? "搜索失败，可手动添加链接" : "尚未执行真实搜索";
  return <section className="research-v2__assistant" aria-label="AI 研究助手"><header><div><p className="industrial-kicker">AI RESEARCH ASSISTANT</p><h2>先把“我该查什么”变成可执行线索</h2><p>真实 Text AI 会根据已确认简报拆解研究路径。它不联网、不写入来源，也不会自动采纳任何事实。</p></div><ResearchBadge tone={tone}>{statusCopy}</ResearchBadge></header>
    {searchUnavailable ? <div className="research-v2__search-config" role="status"><CircleAlert size={16}/><div><strong>真实来源搜索未配置</strong><p>部署管理员需要在 Vercel 环境变量中添加 <code>MUSE_SITE_SEARCH_API_KEY</code>。配置完成前，可以用旁边的“手动搜索”或“添加来源”继续，不会影响项目已保存的内容。</p></div></div> : null}
    {assistant.status === "error" ? <div className="research-v2__assistant-error" role="alert"><CircleAlert size={16}/><span>{assistant.errorMessage || "研究计划暂时没有生成，已有研究内容未被覆盖。"}</span></div> : null}
    {!assistant.questionPlans?.length && assistant.status !== "error" ? <div className="research-v2__assistant-empty"><div><strong>点击上方“生成研究计划”</strong><p>Muse 会为每个研究问题给出检索词、优先来源类型和证据标准，帮助你从模糊想法开始查到真正相关的材料。</p></div><ol><li><span>01</span><strong>AI 拆解问题</strong></li><li><span>02</span><strong>搜索并打开来源</strong></li><li><span>03</span><strong>确认后进入洞察</strong></li></ol></div> : null}
    {assistant.questionPlans?.length ? <div className="research-v2__assistant-plans">{assistant.questionPlans.map((plan, index) => <article key={plan.id}><div className="research-v2__assistant-plan-heading"><span>RQ{String(index + 1).padStart(2, "0")}</span><h3>{plan.question}</h3></div><div className="research-v2__assistant-plan-copy"><div><span>为什么查</span><p>{plan.whyThisMatters}</p></div><div><span>需要什么证据</span><p>{plan.evidenceNeed}</p></div></div><div className="research-v2__assistant-queries"><span>建议检索词</span><div>{plan.querySuggestions.map((query) => <span className="research-v2__query-action" key={query}><button type="button" disabled={Boolean(activeSearch) || searchUnavailable} title={searchUnavailable ? "配置站点搜索后启用" : undefined} onClick={() => void runSearch(query, plan.questionId)}><Search size={12}/>{activeSearch === query ? "搜索中…" : searchUnavailable ? "搜索未配置" : "搜索真实来源"}</button><button type="button" className="is-manual" onClick={() => openManualSearch(query)}><ExternalLink size={12}/>{query}</button></span>)}</div></div><div className="research-v2__assistant-sources"><span>优先来源类型</span><p>{plan.preferredSources.join(" · ")}</p></div></article>)}</div> : null}
    {searchState.status !== "idle" || searchState.query ? <div className="research-v2__search-status" role={searchState.status === "error" ? "alert" : undefined}><div><Search size={14}/><span>{searchStatus}</span>{searchState.provider ? <small>{searchState.provider}</small> : null}</div>{searchState.errorMessage ? <p>{searchState.errorMessage}</p> : null}</div> : null}
    {searchState.results?.length ? <div className="research-v2__search-results"><div className="research-v2__search-results-heading"><span>搜索候选来源</span><small>导入后仍需打开原文核对</small></div>{searchState.results.map((result) => { const imported = importedUrls.has(result.url); return <article key={result.id} className="research-v2__search-result"><div className="research-v2__search-result-main"><a href={result.url} target="_blank" rel="noreferrer"><strong>{result.title}</strong><ExternalLink size={13}/></a><p>{result.publisher}{result.publishedAt ? ` · ${result.publishedAt}` : ""} · {result.contentStatus === "full" ? "已抓取正文" : "仅搜索摘要"}</p><div>{result.snippet}</div></div><button type="button" className="industrial-button" disabled={imported} onClick={() => void importSearchResult(projectId, result, searchState.questionId)}>{imported ? "已导入候选" : "导入候选证据"}<ArrowRight size={14}/></button></article>; })}<p className="research-v2__search-disclaimer">搜索结果只建立候选来源；Muse 不会把摘要自动变成已验证事实。请打开原文、补充必要摘录，再点击“保留”。</p></div> : null}
    {assistant.gaps?.length ? <div className="research-v2__assistant-footer"><div><span>当前信息缺口</span><ul>{assistant.gaps.map((gap) => <li key={gap}>{gap}</li>)}</ul></div>{assistant.nextActions?.length ? <div><span>下一步建议</span><ul>{assistant.nextActions.map((action) => <li key={action}>{action}</li>)}</ul></div> : null}</div> : null}
    <footer className="research-v2__assistant-note"><Sparkles size={15}/><p>{assistant.note}</p><strong>搜索真实来源 → 导入候选 → 打开原文核对 → 你确认采纳 → 生成 Design Insight</strong></footer>
  </section>;
}
...
function EvidenceCard({ item, workspace, projectId }) {
  ...
  const update = useMuseStore((state) => state.updateResearchEvidence);
  const [showDetails, setShowDetails] = useState(false);
  const [editingExcerpt, setEditingExcerpt] = useState(false);
  const [excerptDraft, setExcerptDraft] = useState(item.originalExcerpt || "");
  ...
  const needsOriginalExcerpt = item.contentStatus === "snippet" || !String(item.originalExcerpt || "").trim();
  ...
  const saveExcerpt = async () => {
    const excerpt = excerptDraft.trim();
    if (!excerpt) return;
    await update(projectId, item.id, { originalExcerpt: excerpt, fact: excerpt, contentStatus: "full", verificationStatus: "source_checked", limitations: item.sourceType === "external_search" ? "用户已打开原文并补充摘录；仍需判断该段内容对当前项目的适用范围。" : item.limitations });
    setEditingExcerpt(false);
  };
  return <article className={`research-v2__evidence-card ${isVerified ? "is-verified" : ""}`}>
    <header><div><ResearchBadge tone={tone}>{status}</ResearchBadge><ResearchBadge>{researchSourceTypeLabel(item.sourceType)}</ResearchBadge>{needsOriginalExcerpt ? <ResearchBadge tone="limited">需核对原文</ResearchBadge> : null}</div><span className="research-v2__evidence-date">{item.capturedAt ? new Date(item.capturedAt).toLocaleDateString("zh-CN") : "日期待补充"}</span></header>
    <div className="research-v2__evidence-body"><div className="research-v2__evidence-main"><h3>{item.title}</h3><p className="research-v2__source-name">来源：{item.sourcePublisher || item.sourceName || "未命名来源"}{item.sourceDate ? ` · ${item.sourceDate}` : ""}</p><div className="research-v2__evidence-points"><div><span>原始事实</span><p>{item.fact || item.originalExcerpt || "尚未读取原始内容，不能把链接名称当成事实。"}</p></div><div><span>Muse 解读</span><p>{item.interpretation || "等待 Muse 基于原文整理与项目的关系。"}</p></div><div><span>对设计的影响</span><p>{item.designImplication || "等待人工判断是否会改变设计判断。"}</p></div></div>{needsOriginalExcerpt && !editingExcerpt ? <button type="button" className="research-v2__excerpt-link" onClick={() => { setExcerptDraft(""); setEditingExcerpt(true); }}>我已打开原文，补充原始摘录</button> : null}{editingExcerpt ? <div className="research-v2__excerpt-editor"><label>从原文复制与研究问题直接相关的段落<textarea rows={5} value={excerptDraft} onChange={(event) => setExcerptDraft(event.target.value)} placeholder="粘贴原文摘录，不要只写自己的总结。"/></label><div><button type="button" className="industrial-button" onClick={() => setEditingExcerpt(false)}>取消</button><button type="button" className="industrial-button industrial-button--primary" disabled={!excerptDraft.trim()} onClick={() => void saveExcerpt()}>保存原文摘录</button></div></div> : null}</div></div>
    ...
    <footer><div className="research-v2__linked-questions">{questions.map((question) => <span key={question.id}>{question.id.includes("rq") ? question.id.split("-").slice(-1)[0] : "RQ"}</span>)}</div><div className="research-v2__evidence-actions"><button className="is-primary" disabled={isVerified || needsOriginalExcerpt} onClick={() => void accept(projectId, item.id)}>{isVerified ? "已保留" : needsOriginalExcerpt ? "补充原文后采纳" : "保留"}</button><button onClick={() => void action(projectId, item.id, "reject")}>{item.userStatus === "rejected" ? "已忽略" : "忽略"}</button><button className="is-quiet" onClick={() => setShowDetails((value) => !value)}>{showDetails ? "收起依据" : "查看依据"}<ChevronRight size={13} className={showDetails ? "is-open" : ""}/></button></div></footer>
  </article>;
}
...
export function IndustrialResearchPage() {
  ...
  return <Page project={project} eyebrow="02 · 研究证据" title="找到真正会影响设计判断的信息" description="Muse 先帮你把模糊想法拆成研究线索，再把你确认过的真实来源推进到设计洞察。" showWorkflowConversion={false} actions={<div className="research-v2__header-actions"><button className="industrial-button" onClick={() => setDrawer(true)}><Link2 size={15}/>添加来源</button><PrimaryButton disabled={!insightGate.ready} onClick={() => void enterInsight(false)}>用 {accepted.length} 条证据生成设计洞察</PrimaryButton></div>}>
    <section className="research-v2__entry-guidance"><div><p className="industrial-kicker">AI 辅助研究闭环</p><h2>先生成线索，再确认事实</h2><p>Muse 会把已确认简报拆成检索词、证据标准和信息缺口；真实来源仍由你打开、阅读并确认，避免 AI 把推断包装成事实。</p></div><ol><li><span>01</span><strong>生成研究计划</strong><small>AI 根据每个研究问题给出可执行检索词。</small></li><li><span>02</span><strong>打开真实来源</strong><small>进入检索结果，补充 URL 与原文摘录。</small></li><li><span>03</span><strong>保留可追溯证据</strong><small>只有你确认的材料才会进入 Design Insight。</small></li></ol></section>
    <ResearchAssistantPanel workspace={workspace} projectId={projectId}/>
    {brief?.status !== "confirmed" && !industrial?.briefConfirmed ? <div className="industrial-gate"><LockKeyhole size={18}/>当前简报还未确认；先确认简报，研究问题才会冻结。</div> : null}
