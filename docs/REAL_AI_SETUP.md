...
Muse 的真实生成链路已经接入服务端 BFF（Backend-for-Frontend，前端专用后端）。推荐的生产模式是由站点在服务端保存一枚 Provider（模型服务商）密钥：用户无需输入 Key，浏览器也永远拿不到它；真实请求始终由服务端代理。BYOK（Bring Your Own Key，用户自带密钥）只作为未配置站点托管服务时的可选方案。
...
### Vercel 部署（推荐：站点托管 Text AI）

Vercel 部署通过根目录的 `api/[...path].ts` 暴露同一组 `/api` 接口，并且 SPA 回退不会再拦截 `/api/*`。在 Vercel 项目的 **Settings → Environment Variables** 中配置以下变量，并将它们应用到 Production、Preview（如需预览环境）和 Development（如使用 `vercel dev`）：
...
GitHub Pages 等纯静态托管只适合作为公开作品集展示；它们不会安全地承载供应商密钥。当前 Sites 构建已经把 `/api` 放进 Worker，可支持站点托管或用户自带 Key；若使用其他静态托管，则只能查看已保存案例和离线数据，不执行真实 AI 生成。生产环境仍应配置鉴权、限流、预算和运行数据持久化。
...
没有配置站点 Provider、也没有连接用户 Provider 时，`/api/ai/capabilities` 应明确返回未就绪；这属于安全的未配置状态。启用 BYOK 时，线上 Worker 必须配置 `MUSE_SITE_SECRET`，否则保存 Key 会返回配置错误。
